...
const reactQuestions: QnA[] = [
  ...
  {
    question:
      "SSR(Server-Side Rendering)과 CSR(Client-Side Rendering)의 차이점과 각각의 장단점, 그리고 최신 렌더링 전략에 대해 설명해주세요.",
    answer:
      "SSR은 서버에서 완전한 HTML을 생성하여 클라이언트로 전송하는 방식입니다. 장점으로는 1) 초기 로드 시간 개선(FCP 빠름), 2) 검색 엔진 최적화(SEO) 향상, 3) 소셜 미디어 공유를 위한 메타 태그 제공, 4) 느린 기기나 네트워크에서도 콘텐츠 빠르게 표시가 있습니다. 단점으로는 1) 서버 부하 증가, 2) 복잡한 설정 및 개발, 3) 전체 페이지 새로고침으로 인한 사용자 경험 저하 가능성이 있습니다. CSR은 브라우저에서 JavaScript를 실행하여 DOM을 구성하는 방식입니다. 장점으로는 1) 서버 부하 감소, 2) 페이지 전환 시 빠른 응답성, 3) 풍부한 상호작용, 4) 클라이언트 측 캐싱 활용이 있습니다. 단점으로는 1) 초기 로드 시간 증가, 2) SEO 제한(개선되고 있음), 3) 느린 기기에서 성능 저하가 있습니다. 최신 프레임워크(Next.js, Remix 등)에서는 SSR, CSR 외에도 SSG(Static Site Generation), ISR(Incremental Static Regeneration), Streaming SSR 등 다양한 렌더링 전략을 제공하여 각 페이지나 컴포넌트의 특성에 맞는 최적의 렌더링 방식을 선택할 수 있습니다.",
  },
  {
    question:
      "SPA(Single Page Application)의 개념, 특징, 그리고 구현시 고려사항에 대해 설명해주세요.",
    answer:
      "SPA는 최초 한 번만 페이지를 로드하고 이후에는 필요한 데이터만 동적으로 갱신하는 웹 애플리케이션입니다. React, Vue, Angular 등이 대표적인 SPA 프레임워크입니다. 장점으로는 1) 빠른 사용자 경험과 부드러운 페이지 전환, 2) 서버 부하 감소, 3) 모바일 앱과 유사한 사용자 경험, 4) 프론트엔드와 백엔드의 명확한 분리가 있습니다. 단점으로는 1) 초기 로딩 시간이 길다, 2) SEO가 어렵다, 3) 브라우저 히스토리 관리가 필요하다, 4) 보안 이슈가 있을 수 있다는 점이 있습니다. SPA 구현시 주요 고려사항으로는 1) 코드 스플리팅을 통한 초기 로딩 최적화, 2) 적절한 라우팅 전략(React Router 등 사용), 3) 상태 관리 전략(Redux, MobX 등), 4) SEO 대응(메타 태그 관리, SSR/SSG 고려), 5) 보안(XSS, CSRF 대응), 6) 에러 처리와 로딩 상태 관리, 7) 브라우저 호환성 등이 있습니다.",
  },
  {
  ...
  {
    question:
      "상태 관리 라이브러리(Redux, Recoil, Zustand)의 차이점과 각각의 사용 사례를 설명해주세요.",
    answer:
      "Redux는 가장 성숙한 상태 관리 라이브러리로, Flux 패턴을 기반으로 하며, 예측 가능한 상태 관리와 강력한 미들웨어 생태계를 제공합니다. 특히 큰 규모의 애플리케이션이나 복잡한 상태 로직을 다룰 때 적합하며, Redux Toolkit을 통해 보일러플레이트를 줄일 수 있습니다. Recoil은 Facebook이 개발한 라이브러리로, React의 내부 동작 방식과 잘 어울리며, 비동기 상태 관리와 상태 의존성 관리가 용이합니다. 특히 원자적 상태 관리가 필요하거나 복잡한 상태 의존성이 있는 경우에 적합합니다. Zustand는 최소한의 API로 간단하고 효율적인 상태 관리를 제공하며, Redux와 유사한 패턴을 따르면서도 보일러플레이트가 적습니다. 작은 규모의 프로젝트나 빠른 프로토타이핑에 적합합니다.",
  },
  {
    question:
      "React Query(TanStack Query)의 주요 기능과 사용해야 하는 이유를 설명해주세요.",
    answer:
      "React Query는 서버 상태 관리를 위한 라이브러리로, 캐싱, 동기화, 업데이트를 효율적으로 처리합니다. 주요 기능으로는 자동 캐싱과 재검증, 낙관적 업데이트, 무한 스크롤, 병렬/의존적 쿼리, 자동 재시도 등이 있습니다. 서버 데이터와 클라이언트 상태를 명확히 구분하여 관리할 수 있으며, 로딩/에러 상태 처리가 간편합니다. DevTools를 통한 디버깅이 용이하고, 성능 최적화를 위한 다양한 옵션을 제공합니다. REST API뿐만 아니라 GraphQL도 지원하며, 상태 관리 복잡성을 크게 줄일 수 있습니다.",
  },
  {
    question:
      "React의 상태 관리 방식들(Context API, Redux, MobX)의 차이점과 적절한 사용 시나리오를 설명해주세요.",
    answer:
      "Context API는 React 내장 기능으로, 프롭 드릴링을 피하고 전역 상태를 관리할 수 있습니다. 간단한 상태나 테마, 인증 정보 같은 정적인 데이터에 적합합니다. Redux는 예측 가능한 상태 관리와 강력한 개발자 도구, 미들웨어 생태계를 제공하며, 복잡한 상태 로직이나 대규모 애플리케이션에 적합합니다. MobX는 반응형 프로그래밍 방식을 사용하여 상태 관리를 단순화하며, 보일러플레이트가 적고 유연한 상태 관리가 가능합니다. 작은 규모의 프로젝트나 빠른 개발이 필요한 경우에 적합합니다.",
  },
  {
  ...
  {
    question: "Next.js 14의 주요 특징과 App Router에 대해 설명해주세요.",
    answer:
      "Next.js 14는 서버 컴포넌트를 기본으로 채택하여 성능과 번들 크기를 최적화했으며, App Router를 통해 더 직관적인 파일 시스템 기반 라우팅을 제공합니다. Server Actions를 도입하여 클라이언트-서버 통신을 간소화했고, Partial Prerendering을 통해 정적/동적 렌더링을 효율적으로 혼합할 수 있게 되었습니다. 또한 Turbopack을 통해 개발 환경의 속도를 크게 개선했으며, 타입스크립트 지원과 자동 라우트 타입 생성 등 개발자 경험을 향상시켰습니다. layout.tsx, loading.tsx, error.tsx 등의 특수 파일을 통해 공통 UI를 쉽게 구성할 수 있으며, 서버 컴포넌트와 클라이언트 컴포넌트의 명확한 구분으로 더 나은 아키텍처 설계가 가능해졌습니다.",
  },
  {
    question: "React 18 개발 환경 설정과 주요 기능을 설명해주세요.",
    answer:
      "React 18 개발 환경은 Create React App이나 Vite를 통해 구성할 수 있으며, TypeScript, ESLint, Prettier 등의 도구를 통해 코드 품질을 관리합니다. 주요 기능으로는 동시성 렌더링을 통한 UI 반응성 향상, 자동 배칭을 통한 불필요한 리렌더링 방지, useTransition과 startTransition을 통한 우선순위 기반 렌더링 등이 있습니다. Suspense for Data Fetching을 통해 데이터 로딩 상태를 선언적으로 처리할 수 있으며, 새로운 createRoot API를 통해 더 나은 렌더링 성능을 제공합니다. React DevTools Profiler와 번들 분석 도구를 통해 성능 최적화를 수행할 수 있으며, 서버 컴포넌트를 통해 서버 사이드 렌더링의 효율성을 높일 수 있습니다.",
  },
];
