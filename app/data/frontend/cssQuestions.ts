...
const cssQuestions: QnA[] = [
  ...
  {
    question:
      "Styled-Components와 Tailwind CSS의 차이점과 각각의 장단점을 비교해주세요.",
    answer:
      "Styled-Components와 Tailwind CSS는 서로 다른 스타일링 접근 방식을 가집니다. Styled-Components는 CSS-in-JS 라이브러리로, JavaScript 내에서 CSS를 작성하는 컴포넌트 중심 접근법을 사용합니다. 장점으로는 1) 동적 스타일링이 용이하고, 2) JavaScript 변수와 props를 직접 활용할 수 있으며, 3) 스코프가 격리되어 스타일 충돌이 없고, 4) 컴포넌트와 스타일이 함께 관리되어 유지보수가 쉽습니다. 단점으로는 1) 런타임 오버헤드가 있고, 2) 번들 크기가 증가하며, 3) 별도의 도구 설정이 필요합니다.\n\n" +
      "반면 Tailwind CSS는 유틸리티 우선 접근 방식으로, 미리 정의된 유틸리티 클래스들을 조합하여 스타일을 구성합니다. 장점으로는 1) 빠른 개발이 가능하고, 2) 번들 크기가 최적화되어 있으며, 3) 일관된 디자인 시스템을 쉽게 구축할 수 있고, 4) 런타임 오버헤드가 없습니다. 단점으로는 1) HTML이 길어지고 복잡해질 수 있으며, 2) 동적 스타일링이 상대적으로 불편하고, 3) 초기 학습 곡선이 있습니다.\n\n" +
      "선택 기준은 프로젝트의 요구사항에 따라 다릅니다. 동적 스타일링이 많고 컴포넌트 단위의 스타일 관리가 중요하다면 Styled-Components가, 빠른 개발과 일관된 디자인 시스템이 필요하다면 Tailwind CSS가 더 적합할 수 있습니다. 최근에는 두 방식을 함께 사용하는 하이브리드 접근법도 증가하고 있습니다.",
  },
];
